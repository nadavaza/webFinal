...
class UsersService {
  async login(user: IUser): Promise<IUser> {
    try {
      return (await apiClient.post<IUser>("/auth/login", user ,  { withCredentials: true })).data;
    } catch (error: any) {
      throw error;
    }
  }
  async googleSignin(credentialResponse: CredentialResponse): Promise<IUser> {
    try {
      return (await apiClient.post<IUser>("/auth/googleSignin", credentialResponse ,  { withCredentials: true })).data;
    } catch (error: any) {
      throw error;
    }
  }
  async register(user: IUser): Promise<IUser> {
    try {
      return (await apiClient.post<IUser>("/auth/register", user , { withCredentials: true })).data;
    } catch (error: any) {
      throw error;
    }
  }
  async logout(): Promise<IUser> {
    try {
      return (await apiClient.post("/auth/logout", {} ,  { withCredentials: true })).data;
    } catch (error: any) {
      throw error;
    }
  }
